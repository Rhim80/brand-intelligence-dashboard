...
- needs: 주요 니즈 3-5개
- pain_points: 고충 2-3개
- marketing_strategy: 이 페르소나의 needs와 pain_points를 기반으로 추천 마케팅 전략
  - channel: 추천 마케팅 채널 (예: "맘카페, 육아 커뮤니티")
  - message: 핵심 메시지 (예: "아이 성장에 맞는 가구 솔루션")
  - goal: 마케팅 목표 (예: "성장형 패키지 전환")

키워드 목록:
{kw_lines}

JSON 형식:
{{"clusters": [{{"id": "slug", "persona": "이름", "description": "설명", "keywords": [{{"keyword": "키워드", "volume": 검색량}}], "needs": ["니즈"], "pain_points": ["고충"], "marketing_strategy": {{"channel": "채널", "message": "메시지", "goal": "목표"}}}}]}}"""
...
def save_keyword_clusters(cluster_result, total_relevant):
    ...
    for i, cl in enumerate(cluster_result.get("clusters", [])):
        ...
        # Include marketing_strategy if generated by Gemini
        if cl.get("marketing_strategy"):
            cluster_entry["marketing_strategy"] = cl["marketing_strategy"]
        clusters_output["clusters"].append(cluster_entry)
